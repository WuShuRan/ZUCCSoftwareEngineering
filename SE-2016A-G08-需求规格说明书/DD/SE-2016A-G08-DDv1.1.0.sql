...
INSERT INTO `dictionary` VALUES (1,'气象捕获系统','传感器系统','由一系列传感器组成的环境数据捕获硬件系统','气象捕获系统=温度传感器+湿度传感器+粉尘传感器+雨量计+紫外线传感器+光照传感器+风速传感器+风向传感器','气象捕获系统'),(2,'环境指数','环境数据','由温度、湿度等环境指数构成的数据流','环境数据=温度+湿度+PM2.5浓度+雨量+紫外线指数+光照强度+风速+风向','向云平台传递+云端更新环境数据+下载环境数据+设备更新环境数据+分析环境数据'),(3,'历史环境信息表','环境信息表','保存着所有获取到环境指数的历史信息','环境数据=温度表+湿度表+PM2.5浓度表+雨量表+紫外线指数表+光照强度表+风速表+风向表','云端更新环境数据+设备更新环境数据'),(4,'温度表','','保存着所有获取到温度的历史信息','温度表=0{温度}100000000','环境信息表'),(5,'湿度表','','保存着所有获取到湿度的历史信息','湿度表=0{湿度}100000000','环境信息表'),(6,'PM2.5浓度表','','保存着所有获取到PM2.5浓度的历史信息','PM2.5浓度表=0{PM2.5浓度}100000000','环境信息表'),(7,'雨量表','','保存着所有获取到降雨量的历史信息','雨量表=0{雨量}100000000','环境信息表'),(8,'紫外线指数表','','保存着所有获取到紫外线浓度表的历史信息','紫外线指数表=0{紫外线指数}100000000','环境信息表'),(9,'光照强度表','','保存着所有获取到光照强度表的历史信息','光照强度表=0{光照强度}100000000','环境信息表'),(10,'风速表','','保存着所有获取到风速表的历史信息','风速表=0{风速}100000000','环境信息表'),(11,'风向表','','保存着所有获取到风向表的历史信息','风向表=0{风向}100000000','环境信息表'),(12,'温度','','空气的温度℃','温度=-40{数字}123','温度表'),(13,'湿度','','空气的湿度%RH','湿度=0{百分比}100%','湿度表'),(14,'PM2.5浓度','','24小时平均PM2.5浓度ug/m3','PM2.5浓度=0{数字}500','PM2.5浓度表'),(15,'雨量','','一日内的降雨量mm/d','雨量=0{数字}300','雨量表'),(16,'紫外线指数','','将紫外线强度量化表示','紫外线指数=0{数字}15','紫外线指数表'),(17,'光照强度','','将光照强度强度量化表示lx','光照强度=0{数字}5000','光照强度表'),(18,'空气流动速率','风速','将风速量化表示m/s','风速=0{数字}65','风速表'),(19,'风向','','将风向量化表示m/s','风向=0{数字}360','风向表'),(20,'即时环境信息','','实时的环境信息数据','即时环境信息=温度+湿度+PM2.5浓度+雨量+紫外线指数+光照强度+风速+风向','设备更新环境数据'),(21,'环境-活动表','','保存不同环境下适合做的活动','环境-活动表=0{活动}100','分析环境数据'),(22,'环境-穿衣表','','保存不同环境下适合穿的衣服','环境-穿衣表=0{衣服}100','分析环境数据'),(23,'环境-提示表','','保存不同环境下给出的提示','环境-提示表=0{提示}100','分析环境数据'),(24,'距离','','用户所在位置与气象捕获系统的直线距离m','距离=0{数字}1500','计算距离'),(25,'GPS','','描述气象捕获系统定位的坐标','0{数字,数字}360','气象捕获系统+获取位置信息+计算距离'),(26,'GPS表','','保存着所有GPS的历史信息','0{GPS}100000000','环境信息表');
...
-- Dump completed on 2016-04-08 20:45:49
